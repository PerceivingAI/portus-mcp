...
Each tool maps 1:1 to its permission flag (`subagent_task` requires `subagentTask`, `project_read` requires `projectRead`, etc.). `project_policy` requires `projectPolicy` for checks and its native actions (`register_project`, `list_audit`, and `read_audit`). Its read-only `config` check safely projects `permissionSource: "operator_policy"`, whether the policy selection is `shipped` or `configured`, effective permissions, and redacted path/traversal policy; it never returns the selected absolute policy path. No action accepts permission changes. `requireConfirmation` controls whether mutating operations require `confirm: true` (defaults to `true`). `allowShell` controls whether commands may opt into platform shell execution (defaults to `false`). (`src/policy/permissionPolicy.ts:6-30`, `src/tools/config.ts:51-67`, `src/tools/config.ts:123-192`)
...
After alias selection, scoped `project_context` exposes the selected operator policy's direct-agent execution projection: `enabled`, `allowedCommands`, `allowShell`, and `requireConfirmation`. The project alias supplies project context but never changes permissions. Execution discovery requires `projectContext` rather than administrative `projectPolicy`, and reports permission to attempt a command rather than proof that the executable is installed. (`src/tools/projectBroad.ts:490-503`, `src/policy/policyConfig.ts:130-137`)

Project discovery is not a configuration profile or permission bypass. `project_context` with `include.projects=true` and no `projectAlias` returns registered aliases only; after alias selection, default scoped context includes the selected-policy execution projection. Environment pre-registration through `PORTUS_MCP_PROJECTS` remains operator-side, while model-accessible registration and audit reads are confined to the three native `project_policy` actions and gated by `projectPolicy`. Permissions can be changed only by the operator editing and selecting a complete policy file outside MCP. (`src/tools/projectBroad.ts:460-503`, `src/state/ProjectRegistry.ts:35-103`, `src/tools/config.ts:51-67`)
