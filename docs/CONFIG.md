...
Each tool maps 1:1 to its permission flag: `subagent_task` requires `subagentTask`, `subagent_context` requires `subagentContext`, `project_read` requires `projectRead`, and so on. The two subagent permissions are independent, allowing lifecycle control and read-only session inspection to be granted separately. `project_policy` requires `projectPolicy` for checks and its native actions (`register_project`, `list_audit`, and `read_audit`). Its read-only `config` check safely projects `permissionSource: "operator_policy"`, whether policy selection is `shipped` or `configured`, effective permissions, and redacted path/traversal policy; it never returns the selected absolute policy path. `subagentContext` is a required Boolean in every complete selected policy; older custom policies must add it explicitly, and no fallback to `subagentTask` exists. `requireConfirmation` controls whether mutating operations require `confirm: true`; `allowShell` controls whether commands may opt into platform shell execution. (`src/policy/policyConfig.ts:39-53`, `src/tools/subagents.ts:206-216`, `src/tools/config.ts:181-203`)
