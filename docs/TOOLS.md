...
| `project_screenshot` | Discover visible top-level windows of a running execution session (`discover_running`), launch a command and capture its window with lifecycle management (`capture_launch`), capture an existing running session window (`capture_running`), or read, list, and explicitly delete repository-local PNG/JPEG captures (`read`, `list`, `delete`). |
...
Every executed and skipped item result includes a deterministic `lifecycle` object with explicit command termination ownership booleans: `processStarted`, `processExited`, `killAttempted`, `killSucceeded`, `waitAttempted`, and `reaped`. When Portus initiates process-tree termination, `lifecycle` additionally reports `terminationOutcome` (`terminated`, `termination_unverified`, or `termination_failed`), `terminationVerification` (`confirmed_absent`, `confirmed_alive`, or `unavailable`), `descendantsRemaining`, `scope`, and the platform method. `terminationActionError` records a failed termination command or fallback separately from `terminationVerificationError`. Reliable confirmation that the tree is absent produces `terminated` even when the action reported a warning; unavailable verification produces `termination_unverified`. Native PIDs are never exposed through lifecycle results.

### `project_run` Observable Execution Sessions Contract

For long-running tasks (e.g. mathematical replays, large builds, tests, or interactive workers), `project_run` supports asynchronous execution sessions via `sessionAction`:
...
3. **`write`**: `{ sessionAction: { type: "write", sessionId: "...", input: "status\\n" } }` writes at most 64 KiB of UTF-8 input to the running process's stdin and returns the session status and `writtenBytes`. Writes fail when the session is no longer running or stdin is unavailable.
4. **`terminate`**: `{ sessionAction: { type: "terminate", sessionId: "..." } }` gracefully terminates and reaps the entire process tree for that session.
5. **`list`**: `{ sessionAction: { type: "list" } }` lists execution sessions for the project, newest first.

### `project_screenshot` contract

Enable the tool with `main_agent.permissions.projectScreenshot`. Every request uses a strict discriminated operation union. `app_discovery` lists configured GUI applications found on the host without launching them; `discover_running` and `capture_running` require a running execution session whose live process identity can be attested; `capture_launch` spawns a command and captures its window; `read`, `list`, and `delete` remain available for persisted captures after the session exits. (`src/tools/projectScreenshot.ts`, `src/runtime/appDiscovery.ts`, `src/runtime/screenshotSystem.ts`)

| Operation | Behavior |
|---|---|
| `app_discovery` | Return one flat, deduplicated list of usable names from `screenshot.appDiscovery.commands` and `.aliases` without launching a process or exposing resolved paths. Aliases win duplicate names. Requires `projectAlias`. |
| `discover_running` | Return only visible, non-minimized windows owned by the selected execution session. Results contain opaque, short-lived `windowId` tokens, never PIDs or native handles. Requires `projectAlias` and `executionSessionId`. |
| `capture_launch` | Launch a command via `startExecutionSession` and capture its GUI window. Commands in `screenshot.appDiscovery.commands` use automatic path resolution; names in `.aliases` use their configured absolute path and override duplicate commands without fallback. Either group authorizes direct launch without `main_agent.permissions.allowedCommands`. Commands absent from both groups retain the existing allowlist-controlled launch path. Requires `projectAlias`, `command`, and `closeSession`. Supports optional launch `args`, `shell`, `waitForWindowMs` (default 5000), `format`, `jpegQuality`, `maxWidth`, `maxHeight`, `returnImage`, and `confirm`; configured entries reject `shell: true`. |
| `capture_running` | Capture the sole eligible window or a selected `windowId` for an existing session; save PNG or JPEG under `.portus-artifacts/screenshots/<executionSessionId>/`; return metadata and a native MCP image block unless `returnImage=false`. Requires `projectAlias`, `executionSessionId`, and `closeSession: boolean`. Supports optional `windowId`, `waitForWindowMs` (default 0), `format`, `jpegQuality`, `maxWidth`, `maxHeight`, `returnImage`, and `confirm`. |
| `read` | Revalidate and return one managed screenshot from the same project/session, including a native image block unless `returnImage=false`. Requires `projectAlias`, `executionSessionId`, and `screenshotId`. |
| `list` | Return newest-first managed screenshot metadata using bounded, opaque cursor pagination. Requires `projectAlias` and `executionSessionId`. |
| `delete` | Explicitly remove one managed screenshot from the same project/session. Requires `projectAlias`, `executionSessionId`, `screenshotId`, and `confirm` if policy requires. |

Both `capture_launch` and `capture_running` require `closeSession: boolean` (to auto-close the GUI/session or leave it running), and support bounded `waitForWindowMs`, `format`, `jpegQuality`, `maxWidth`, and `maxHeight`. In `capture_running`, if several eligible windows exist and no `windowId` is supplied, the error is `multiple_session_windows` and includes scoped candidates. `capture_launch`, `capture_running`, and `delete` require `confirm=true` when `main_agent.permissions.requireConfirmation=true`. Windows, macOS, and Linux X11 use the isolated npm-installed worker; Wayland fails closed with `unsupported_session_window_capture`. (`scripts/screenshot-worker.mjs`, `src/runtime/screenshotSystem.ts`)
