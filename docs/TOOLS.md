...
Enable the tool with `main_agent.permissions.projectScreenshot`. Every request uses a strict discriminated operation union. `discover_running` and `capture_running` require a running execution session whose live process identity can be attested; `capture_launch` spawns a command and captures its window; `read`, `list`, and `delete` remain available for persisted captures after the session exits. (`src/tools/projectScreenshot.ts`, `src/runtime/screenshotSystem.ts`)
...
| `capture_launch` | Launch a command via `startExecutionSession` and capture its GUI window. Requires `projectAlias`, `command`, and `closeSession` (to auto-close the GUI/session or leave it running). Supports optional `args`, `shell`, `waitForWindowMs` (default 5000), `format`, `jpegQuality`, `maxWidth`, `maxHeight`, `returnImage`, and `confirm`. |
