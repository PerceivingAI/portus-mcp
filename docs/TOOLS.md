...
| `project_edit` | Run ordered write, exact replace, unique-marker insert, hash-guarded inclusive `replace_range`, copy, move, delete, mkdir, or rmdir operations, optionally as a dry run. `replace` requires a positive `expectedOccurrences`; `insert` requires exactly one marker; `replace_range` requires the complete-file SHA-256 returned by `project_read`, uses one-based inclusive lines, and is capped by `limits.textEdit.maxRangeLines`. Results distinguish applied, no-change, planned, rejected, failed, and skipped operations. Batches stop on the first unsuccessful operation unless `continueOnFailure=true`; they are ordered but not atomic. |
