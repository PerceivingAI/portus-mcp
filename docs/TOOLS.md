...
| `project_edit` | Run ordered write, exact replace, unique-marker insert, copy, move, delete, mkdir, or rmdir operations, optionally as a dry run. `replace` requires a positive `expectedOccurrences`; `insert` requires exactly one marker. Results distinguish applied, no-change, planned, rejected, failed, and skipped operations. Batches stop on the first unsuccessful operation unless `continueOnFailure=true`; they are ordered but not atomic. |
